...
enum
{
#define _(var, id, name) IPV4_UDP_SESSION_LOOKUP_NEXT_##id,
	foreach_ipv4_udp_session_lookup_next
#undef _
	IPV4_UDP_SESSION_LOOKUP_NEXT_N,
};
...
enum
{
#define _(id, name) IPV4_UDP_SESSION_LOOKUP_##id,
	foreach_detunnel_counter
#undef _
	IPV4_UDP_SESSION_LOOKUP_COUNTER_N,
};

typedef struct
{
	ipv4_flow_key_t key;
} ipv4_udp_session_lookup_trace_t;

typedef struct
{
} ipv4_udp_session_lookup_main_t;
...
extern vlib_node_registration_t ipv4_udp_timer_expiration_process;

static_always_inline void
ipv4_udp_session_lookup_to_next(u16 *next, u16 len)
{
		...
		const ipv4_flow_key_t *key)
{
	...
	{
		ipv4_udp_session_lookup_trace_t *t = vlib_add_trace(vm, node, b, sizeof(*t));
		t->key = *key;
	}
}
...
ipv4_udp_session_lookup_inline(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame, u8 is_trace)
{
	...
	ipv4_udp_session_lookup_to_next(nexts, frame->n_vectors);
	vlib_buffer_enqueue_to_next(vm, node, from, nexts, frame->n_vectors);

	return frame->n_vectors;
}

VLIB_NODE_FN (ipv4_udp_session_lookup) (vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame)
{
	return ipv4_udp_session_lookup_inline(vm, node, frame, node->flags & VLIB_NODE_FLAG_TRACE);
}
...
VLIB_NODE_FN (ipv4_udp_timer_expiration_process) (vlib_main_t *vm, vlib_node_runtime_t __clib_unused *node,
		vlib_frame_t __clib_unused *frame)
{
	...
	while (tr->count > 0)
	{
		...
		for (u32 thread_id = start_thread_id; thread_id < end_thread_id; thread_id++)
			vlib_node_set_interrupt_pending(vlib_get_main_by_index(thread_id), ipv4_udp_timer_expiration.index);
	}
...
static u8 *format_ipv4_udp_session_lookup_trace(u8 *s, va_list *args)
{
	...
	ipv4_udp_session_lookup_trace_t *t = va_arg(*args, ipv4_udp_session_lookup_trace_t *);
...
VLIB_REGISTER_NODE (ipv4_udp_session_lookup) = {
	.name = "ipv4-udp-session-lookup",
	.vector_size = sizeof(u32),
	.format_trace = format_ipv4_udp_session_lookup_trace,
	.type = VLIB_NODE_TYPE_INTERNAL,
	.n_next_nodes = IPV4_UDP_SESSION_LOOKUP_NEXT_N,
	.next_nodes = {
#define _(var, id, name) [IPV4_UDP_SESSION_LOOKUP_NEXT_##id] = (name),
	foreach_ipv4_udp_session_lookup_next
#undef _
	},
};

VLIB_REGISTER_NODE (ipv4_udp_timer_expiration) = {
	.name = "ipv4-udp-timer-expiration",
	.type = VLIB_NODE_TYPE_SCHED,
	.state = VLIB_NODE_STATE_INTERRUPT
};

VLIB_REGISTER_NODE (ipv4_udp_timer_expiration_process) = {
	.name = "ipv4-udp-timer-expiration-process",
	.type = VLIB_NODE_TYPE_PROCESS,
};

#endif

CLIB_MARCH_FN (ipv4_udp_session_lookup_init, clib_error_t *, vlib_main_t __clib_unused *vm)
{
	clib_warning("size: %lu %s", SIMD_SIZE, CLIB_STRING_MACRO(SIMD_TYPE));

	SIMD_VEC(drop_next) = SIMD_SPLAT(IPV4_UDP_SESSION_LOOKUP_NEXT_DROP);

	return 0;
}

static void
ipv4_udp_session_expired_timer_callback(u32 *session_indexes)
{
...
ipv4_udp_session_lookup_worker_init(vlib_main_t __clib_unused *vm)
{
	...
	tw_timer_wheel_init_1t_3w_1024sl_ov(&sw->time_wheel, ipv4_udp_session_expired_timer_callback, 1.0, ~0);
...
VLIB_WORKER_INIT_FUNCTION (ipv4_udp_session_lookup_worker_init);
VLIB_INIT_FUNCTION (ipv4_udp_session_lookup_init);