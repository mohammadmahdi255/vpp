...
enum
{
#define _(var, id, name) SESSION_V6_LOOKUP_NEXT_##id,
	foreach_session_v6_lookup_next
#undef _
	SESSION_V6_LOOKUP_NEXT_N,
};
...
session_v6_lookup_to_next(u16 *next, u16 len)
{
...
flow_key_v6_direction(const ip6_header_t *ip6, const nat_tcp_udp_header_t *l4)
{
	return l4->src_port > l4->dst_port || ip6->src_address.as_u64[0] > ip6->dst_address.as_u64[0]
			|| ip6->src_address.as_u64[1] > ip6->dst_address.as_u64[1];
}

static_always_inline session_direction_t
session_v6_direction(const flow_key_v6_t *key)
{
	...
	const ip6_address_t *src_ip = &key->ip[FLOW_DIRECTION_ORIGINAL];
	const ip6_address_t *dst_ip = &key->ip[FLOW_DIRECTION_REVERSE];

	return src_port > dst_port || memcmp(src_ip->as_u8, dst_ip->as_u8, sizeof(ip6_address_t)) > 0;
}
...
process_buffer_1x(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_buffer_t *b, u16 *next, u8 is_trace)
{
	...
	if (vt_is_end(it))
	{
		...
		session->session_direction = session_v6_direction(&key);
...
session_v6_lookup_inline(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame, u8 is_trace)
{
	...
	session_v6_lookup_to_next(nexts, frame->n_vectors);
	vlib_buffer_enqueue_to_next(vm, node, from, nexts, frame->n_vectors);

	return frame->n_vectors;
}

VLIB_NODE_FN (session_v6_lookup) (vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame)
{
		...
		vlib_frame_t __clib_unused *frame)
{
	...
	session_v6_lookup_main_t *sm = &session_v6_lookup_main;
	session_worker_t *sw = session_worker;
	tw_timer_wheel_1t_3w_1024sl_ov_t *tw = &sw->time_wheel_v6;
	session_t *session;
	u32 n_remove = 0;
	i32 rv;
	...
	for (u32 i = 0; i < n_expire; i++)
	{
		...
		if (session->end_time - sw->now > tc->resolution)
		{
			...
			tw_timer_start_1t_3w_1024sl_ov(&sw->time_wheel_v6, session_index, 0, timeout);
			continue;
		}

		vt_erase(&sw->session_map_v6, session->key_v6);
		...
		buffer = produce_v6_csv_record(vm, session, buffer);
		msg.payload = buffer;
...
VLIB_REGISTER_NODE (session_v6_lookup) = {
	.name = "session-v6-lookup",
	.vector_size = sizeof(u32),
	.format_trace = format_session_v6_lookup_trace,
	.type = VLIB_NODE_TYPE_INTERNAL,
	.n_next_nodes = SESSION_V6_LOOKUP_NEXT_N,
	.next_nodes = {
#define _(var, id, name) [SESSION_V6_LOOKUP_NEXT_##id] = (name),
	foreach_session_v6_lookup_next
#undef _
	},
};
...
CLIB_MARCH_FN (session_v6_lookup_init, clib_error_t *, vlib_main_t __clib_unused *vm)
{
	...
	simd_u16(drop_next) = simd_u16_splat(SESSION_V6_LOOKUP_NEXT_DROP);
