...
init_session_v4_direction(const flow_key_v4_t *key, const flow_direction_t flow_direction)
{
...
process_buffer_1x(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_buffer_t *b, u16 *next, u8 is_trace)
{
	...
	if (vt_is_end(it))
	{
		...
		it = session_v4_map_insert_raw(&sw->session_map_v4, key, &session, true, true);
		...
		session->session_direction = init_session_v4_direction(&key, fd);
