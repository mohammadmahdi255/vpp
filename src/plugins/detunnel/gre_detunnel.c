...
gre_to_next(u16 *next, u16 len)
{
	for (u16 i = 0; i < len; i += simd_u16_size)
	{
		...
		simd_u16_t ethernet_mask_vec = (next_vec == simd_u16(eoip_ethertype));
		...
		simd_u16_t result = simd_u16(drop_next) |
				(ethernet_mask_vec & simd_u16(ethernet_next)) |
...
CLIB_MARCH_FN (gre_detunnel_init, clib_error_t *, vlib_main_t __clib_unused *vm)
{
	...
	simd_u16(drop_next) = simd_u16_splat(GRE_NEXT_DROP);
	simd_u16(ethernet_next) = simd_u16_splat(GRE_NEXT_ETHERNET_DETUNNEL);
	simd_u16(vlan_next) = simd_u16_splat(GRE_NEXT_VLAN_DETUNNEL);
	simd_u16(ipv4_next) = simd_u16_splat(GRE_NEXT_IPV4_DETUNNEL);
	simd_u16(ipv6_next) = simd_u16_splat(GRE_NEXT_IPV6_DETUNNEL);
	simd_u16(mpls_next) = simd_u16_splat(GRE_NEXT_MPLS_DETUNNEL);
	simd_u16(pppoe_next) = simd_u16_splat(GRE_NEXT_PPPOE_DETUNNEL);
	simd_u16(ppp_next) = simd_u16_splat(GRE_NEXT_PPP_DETUNNEL);
	simd_u16(failed_next) = simd_u16_splat(GRE_NEXT_FAILED_DETUNNEL);
