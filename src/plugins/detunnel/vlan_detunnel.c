...
} vlan_detunnel_worker_t;

extern __thread vlan_detunnel_worker_t vlan_detunnel_worker;
...
process_buffer_1x(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_buffer_t *b, u16 *next, u8 is_trace)
{
	...
	vdw->counters[sw_idx].packets++;
	vdw->counters[sw_idx].bytes += sizeof(vlan_header_t);
	next[0] = vlan->type;
...
vlan_detunnel_inline(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame, u8 is_trace)
{
	...
	{
		vlib_counter_t *counter = &vdw->counters[sw_idx];
...
__thread vlan_detunnel_worker_t vlan_detunnel_worker;
...
CLIB_MARCH_FN (vlan_detunnel_init, clib_error_t *, vlib_main_t __clib_unused *vm)
{
	...
	simd_u16(drop_next) = simd_u16_splat(VLAN_NEXT_DROP);
	simd_u16(vlan_next) = simd_u16_splat(VLAN_NEXT_VLAN_DETUNNEL);
	simd_u16(ipv4_next) = simd_u16_splat(VLAN_NEXT_IPV4_DETUNNEL);
	simd_u16(ipv6_next) = simd_u16_splat(VLAN_NEXT_IPV6_DETUNNEL);
	simd_u16(mpls_next) = simd_u16_splat(VLAN_NEXT_MPLS_DETUNNEL);
	simd_u16(pppoe_next) = simd_u16_splat(VLAN_NEXT_PPPOE_DETUNNEL);
	simd_u16(ppp_next) = simd_u16_splat(VLAN_NEXT_PPP_DETUNNEL);
	simd_u16(failed_next) = simd_u16_splat(VLAN_NEXT_FAILED_DETUNNEL);

	return 0;
}

static clib_error_t *
vlan_detunnel_worker_init(vlib_main_t __clib_unused *vm)
{
	vlan_detunnel_worker_t *vdw = &vlan_detunnel_worker;
	clib_memset(vdw->counters, 0, sizeof(vdw->counters));
	return 0;
}
...
VLIB_WORKER_INIT_FUNCTION (vlan_detunnel_worker_init);
